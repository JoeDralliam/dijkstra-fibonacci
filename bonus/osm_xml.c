...
int cmp_node(const void* node1, const void* node2)
{
    const osm_node_t* n1 = *(osm_node_t**)node1;
    const osm_node_t* n2 = *(osm_node_t**)node2;
    return n1->id - n2->id;
}
...
{
    size_t start = 0;
    size_t end = array_length(&nodes);
    while(end > start)
    {
	size_t mid = (start + end) / 2;
	osm_node_t* node = array_get(&nodes, mid);
	if(node->id < refI)
	{
	    start = mid + 1;
	}
	else if(node->id > refI)
	{
	    end = mid;
	}
	else
...
graph_t parse_osm(char const* filename, gen_array* nodes)
{
    *nodes = read_osm_nodes(filename);
    qsort(nodes->array, nodes->used, sizeof(void*), &cmp_node);
